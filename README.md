...
The level screen also offers two rulesets. **Standard** is the existing falling-block game. **Flowing** opens with a centered two-cell target and a matching domino for an immediate first clear. The patterned floor and active piece then scroll steadily upward together while incomplete rows enter continuously from below. If the floor reaches the ceiling, the run ends. When a placed piece completes a line, its surviving cells fall independently; any line they complete clears as a score-multiplied cascade before the next piece appears. The floor masks and flow speed are centralized in `scripts/game_config.gd` for later tuning.
