...
The level screen also offers two rulesets. **Standard** is the existing falling-block game. **Flowing** opens with a centered two-cell target and a matching domino for an immediate first clear. The patterned floor and active piece then scroll steadily upward together while a two-cell clearing channel zigzags left and right through incoming rows. That channel moves only one column per row, preserving a connected cave at least one full block wide throughout the pattern and across its loop. If the floor reaches the ceiling, the run ends. Every placed piece separates into cells that fall independently through unsupported openings before lines are checked; surviving cells settle again after a clear and can produce score-multiplied cascades. Cleared blocks break into eight tumbling shards in a left-to-right sweep. The floor masks and flow speed are centralized in `scripts/game_config.gd` for later tuning.
