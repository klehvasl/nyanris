...
- Hard drop remains logically instantaneous and adds three piece-shaped afterimages plus a 120 ms squash → slight overshoot → settle. The broad flash is replaced by a one-frame contact line and eight deterministic 1–2 px fragments; no particle node or texture asset is used.
...
- A temporary title-screen − / + selector chooses the starting level from 0 through 9. It supports click/touch, Left/Right or A/D, and direct number keys 0–9. This is intentionally structured to be upgraded into the final level-selection screen later.
...
4. Board/UI: functional themed UI in place; supplied sheet pending production slicing.
5. Cat + FX + audio: idle and four-line reaction plus lightweight synthesized feedback integrated; final art-driven FX and authored sound assets pending.
